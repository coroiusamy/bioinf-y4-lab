...
def main():
    ...
    print(f"\n{'='*70}")
    print("=== Rezultat Aliniere Globală (NW) ===")
    print(f"{'='*70}")
    print(f"Scor final: {sc}")
    print(f"Lungime aliniere: {len(a1)}")
    ...
    print(f"  Gaps: {gaps}")
    print(f"  Identitate: {identity:.2f}%")
    
    # Afișăm primele 200 caractere din aliniere
    print(f"\nPrimele 200 caractere din aliniere:")
    print(f"{id1[:10]:10s} {a1[:200]}")
    match_line = ''.join(['|' if x == y else ' ' for x, y in zip(a1[:200], a2[:200])])
    print(f"{'':10s} {match_line}")
    print(f"{id2[:10]:10s} {a2[:200]}")
    print(f"{'='*70}\n")
